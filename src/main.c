...
char	wall_selection(double angle, char c)
{
	...
	else if (angle > FULL_CIRCLE_DEGREES / 4 && angle <= FULL_CIRCLE_DEGREES
			/ 2)
		return (wall_helper(c, 'E', 'N'));
	else if (angle > FULL_CIRCLE_DEGREES / 2 && angle <= FULL_CIRCLE_DEGREES * 3
			/ 4)
	...
	if (same(ray_angle, 270) || same(ray_angle, 90))
	{
		while (temp_pos)
		{
			if (temp_pos->sn_winner_texture == flag
				&& same_double(temp_pos->sn_winner_x, all->x))
				if (!winner
					|| (temp_pos->sn_winner_distance < winner->sn_winner_distance))
					winner = temp_pos;
			temp_pos = temp_pos->next;
	...
	else if (same(ray_angle, 0) || same(ray_angle, 180))
	{
		while (temp_pos)
		{
			if (temp_pos->ew_winner_texture == flag
				&& same_double(temp_pos->ew_winner_y, all->y))
				if (!winner
					|| (temp_pos->ew_winner_distance < winner->ew_winner_distance))
					winner = temp_pos;
			temp_pos = temp_pos->next;
...
double	find_lateral_distance(t_all *all, char flag)
{
	...
	if (final.texture == final.pos->ew_winner_texture)
		intersection_distance = distance(all->x, all->y, final.pos->ew_winner_x,
				find_y(ray_angle, all->x, all->y, final.pos->ew_winner_x));
	else
		intersection_distance = distance(all->x, all->y, find_x(ray_angle,
					all->x, all->y, final.pos->sn_winner_y),
...
t_render	create_render_data(t_winner final, double intersection_distance,
		double temp_angle)
{
