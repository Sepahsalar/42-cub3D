...
/*   Updated: 2024/08/15 15:05:40 by asohrabi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return ((temp->nx != -1 && temp->ny != -1)
			&& sqrt(pow(all->x - temp->nx, 2) + pow(all->y - temp->ny, 2))
				< temp->sn_winner_distance);
}

int	west_is_better_than_east(t_all *all, t_loc *temp)
{
	return ((temp->wx != -1 && temp->wy != -1)
			&& sqrt(pow(all->x - temp->wx, 2) + pow(all->y - temp->wy, 2))
				< temp->ew_winner_distance);
}
